% spikes2spikes_asg         compute s2s structure with ASG and STC information
%
...
if sum( nidx )
    ...
    gbUpper                     = poissinv( 1 - alfa / nBonfD, max( pred( t_ROI, : ), [], 1 ) );
    gbLower                     = poissinv( alfa / nBonfD, min( pred( t_ROI, : ), [], 1 ) );
    
    % detect the bins with high/low global counts
    hiBins                      = false( size( cchD ) );
    loBins                      = false( size( cchD ) );
    hiBins( t_ROI, : )          = bsxfun( @gt, cchD( t_ROI, : ), gbUpper ) & ( cchD( t_ROI, : ) > 0 );
    loBins( t_ROI, : )          = bsxfun( @lt, cchD( t_ROI, : ), gbLower ) & ( ones( nBonfD, 1 ) * gbLower ) > 0;
    nans                        = NaN( sum( nidx ), sum( idx0 ) );
    
    % determine if any bin in t_ROI is globally sig. (binary decision)
    act0                       	= any( cchD( t_ROI, : ) >= ones( sum( t_ROI ), 1 ) * gbUpper, 1 );
    sil0                        = any( cchD( t_ROI, : ) <= ones( sum( t_ROI ), 1 ) * gbLower, 1 );
...
tic, s2s_new = spikes2spikes_asg( filebase, 'Overwrite', 0 ); toc
tic, s2s_old = spikes2spikes( filebase, 'Overwrite', 0 ); toc
