...
% next steps (08-jul-20):
% (1) support mode 2                                                        done 08-jul-20
% (2) test without burst filtering, and implement deconvolution
% (3) add computation of SG50, CA
% (4) make sure the specifics for spike selection & predictor calculations
% are the same for this routine (mode 3) and spikes2spikes
% (5) implement resampling for SEM

% input configurations:
% (1) s2s, n1, n2 (assumes s2s has fields: ccg, shankclu, t, nspks, pred, t_ROI)
% (2) the actual CCH, nspks1, dt, predictor (or W), t_ROI, (ACH)
...
[ BinSizeMS, halfWidthMS, jitWindowMS, SpikesFs ...
    ...
    , ach1, ach2, t, periods, cmode, graphics ] = ParseArgPairs( ...
    { 'BinSizeMS', 'halfWidthMS', 'jitWindowMS', 'SpikesFs' ...
    , 'roiMS', 'convType', 'alpha0', 'deadTimeMS', 'CutOffMS' ...
    , 'ach1', 'ach2', 't', 'periods', 'cmode', 'graphics' } ...
    , { 1, 50, 5, 20000 ...
    , [ 0 5 ], 'gauss', 0.001, 0.4, 5 ...
    , [], [], [], [], 1, 0 }...
    , varargin{ : } );
...
% 1. get basic parameters (cch, cchbins, nspks1, dt, alfa, t_ROI
switch cmode
    ...
    case 3 % compute raw cch from spike trains (diluted according to periods)
        ...
        res                     = res( sidx );
        uClu                    = unique( clu );
        [ ccg, tt, pairs ]      = CCG( res, clu, BinSize, nBins, SpikesFs, uClu, 'count' );
        cch                     = ccg( :, 1, 2 );
        ach1                    = ccg( :, 1, 1 );
        ach2                    = ccg( :, 2, 2 );
...
switch cmode
    ...
    case { 2, 3 } % compute based on cch
        ...
        nidx                    = t <= deadTimeMS & t >= -deadTimeMS;
        pred( nidx )            = NaN;
end
...
%------------------------------------------------------------
% mode #2 (from the cch and the ach)
...
[ g1, g2, act, sil, s ] = calc_asg( cch, nspk1, 'cmode', 2, 'graphics', 1 );
...
ridx                    = inranges( s.res, uvals );
s.clu( ridx )           = [];
s.res( ridx )           = [];

% get the spike times of the trigger
clunum1                 = s.map( ismember( s.map( :, 2 : 3 ), shankclu1, 'rows' ), 1 );
clunum2                 = s.map( ismember( s.map( :, 2 : 3 ), shankclu2, 'rows' ), 1 );
st1                     = s.res( s.clu == clunum1 );
st2                     = s.res( s.clu == clunum2 );

[ g1, g2, act, sil, s ] = calc_asg( st1, st2, 'cmode', 3, 'graphics', 1 );
[ g1, g2, act, sil, s ] = calc_asg( st1, st2, 'cmode', 3, 'graphics', 1, 'BinSizeMS', 0.1 );
...
s2s                                 = struct_select( s2s, kidx2, 1 );
s2s                                 = struct_select( s2s, kidx2, 2 );
s2s                                 = struct_select( s2s, kidx2, 3 );
