...
% 02-jan-13 small modification: use unblank() instead of udeblank(deblank())
...
while ~eot & ~isempty( unblank( str ) )
  ...
  if f_end <= f_beg
    ...
    z.value   = unblank(sprintf('%s %s',current_value, str_t));
    eot       = 1;
    %>
  else
