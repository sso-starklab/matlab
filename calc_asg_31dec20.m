...
%                   act                 flag, 1 if any CCH bin in the ROI is sig. above the predictor
...
% 17-dec-20 (1) yet another refinement of asgMode 1: case of consecutive
%               identical values handled
% 31-dec-20 (1) yet another refinement of asgMode 1: 
% - if no local maximum, but the global maximum is at the far end of the
% ROI (far from zero-lag), then use asgMode 0 (and the mirro for the asg2)
...
switch asgMode
    ...
    case 1
        ...
        x                       = cch( xidx );
%         maxidx                  = find( diff( sign( diff( x ) ) ) < -1 );
        % the above is great (at indices of ft_ROI) if all values of x are unique  
        % however, if they are not (e.g. strong inhibition, or fixed excitation)
        %   there will be no local maximum. for that, we need to collapse
        %   multiple identical values into a single value first
...
switch asgMode
    case 0
        [ ~, minidx ]           = min( x );
    case 1
%        minidx                  = find( diff( sign( diff( x ) ) ) > 1 );
        % the above is great (at indices of ft_ROI) if all values of x are unique  
        % however, if they are not (e.g. strong inhibition, or fixed excitation)
        %   there will be no local minimum. for that, we need to collapse
        %   multiple identical values into a single value first
        tmpidx                  = find( diff( sign( diff( ux ) ) ) > 1 ) + 1;
        minidx                  = ix( tmpidx ) - 1;
end
