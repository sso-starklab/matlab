...
%--------
% 30-dec-19, first part of adding separatices:
%--------
% sstN = struct_select( sst, ~ispos );
% labels = sstN.pyr;
% param1 = sstN.tp; 
% param2 = sstN.geo_fwhm;
% param2 = sstN.ach_com;
% for i = 1 : 2
%     ct = i - 1;
%     idx = labels == ct;
...
% xres = 0.01;
% yres = 0.1;
% xvals = xlims( 1 ) : xres : xlims( 2 );
% yvals = ylims( 1 ) : yres : ylims( 2 );
% xmat = xvals( : ) * ones( 1, length( yvals ) );
% ymat = ones( length( xvals ), 1 ) * yvals( : ).';
% grid = [ xmat( : ) ymat( : ) ];
% p = posterior( gm, grid );
% [ ~, cls ]          = max( p, [], 2 );
% figure
% plot( grid( cls == 1, 1 ), grid( cls == 1, 2 ), '.b', grid( cls == 2, 1 ), grid( cls == 2, 2 ), '.r' )
%--------
