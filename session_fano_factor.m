...
% 29-dec-19 (1) added computation of overall mean rates
% 04-jun-20 (1) if no periods, return
%           (2) if no ct, continue
...
if isempty( periods )
    fprintf( 1, '%s: empty periods for %s\n', upper( mfilename ), filebase )
    return
end
...
s.nwindows          = nwindows;
s.winsizes          = wsvec;
totaldur            = sum( diff( periods, [], 2 ) + 1 ) / spkFs;        % [s]
s.baseline          = nspikes / totaldur;
...
filebase = '/odin3/Recordings/mP23/dat/mP23_25/mP23_25';
[ ~, filename ] = fileparts( filebase );
[ fanomat, s ] = session_fano_factor( filebase, 'brain_state', 'SWS', 'graphics', 0);
if ~isempty( fanomat )
	savename = [ savepath '/' filename '_fanofactors_SWS.mat' ];
...
for i=1:length(x)
    try
        filebase = sprintf('/odin3/Recordings/mDS2/dat/mDS2_%d/mDS2_%d', x(i),x(i));
        [ ~, filename ] = fileparts( filebase );
        [ fanomat, s ] = session_fano_factor( filebase,'brain_state', 'SWS', 'graphics', 0 );
        if ~isempty( fanomat )
            savename = [ savepath '/' filename '_fanofactors_SWS.mat' ];
        ...
        [ fanomat, s ] = session_fano_factor( filebase , 'brain_state', 'theta','graphics', 0 );
        savename = [ savepath '/' filename '_fanofactors_theta.mat' ];
        save( savename, 's', 'fanomat' );
    end
end