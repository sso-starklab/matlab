...
function [ vA, vT, bpi, tV, vB, pA, nA, pT, nT, upol, tp, rA, abvTH ] = calc_spatial_waveform_features_28jun22( w, TH, spkFs, graphics,varargin)
...
if size (TH,1)>1
    for ch = 1:length(vA)
        if vA(ch)<0
            vidx(ch)                        = abs( vA(ch) ) >= 2*TH(ch,1);
        else
            vidx(ch)                        = abs( vA(ch) ) >= 2*TH(ch,2);
        end
    end
else
    vidx                            = abs( vA ) >= TH;
