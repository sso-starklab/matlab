...
% 14-jul-14 mode - 'standard' or 'diff'
...
function xd = dehighpass( x, minAmp, minDur, f, mode, g )
...
%     g = 0;
% end
if nargs < 5 || isempty( mode )
    mode = 'standard';
end
mode = lower( mode );
if nargs < 6 || isempty( g )
    g = 0;
end

% partition into segments/trains
if strcmp( mode, 'diff' )
    x2 = abs( derive( x ) );
    mat = parseSchmitt( x2, minAmp( 1 ), minAmp( 2 ) );      % events
    mat = dilutesegments( mat, 0, minDur );
    ...
    for i = 1 : m
        np = T( i );
        mm = x( mat( i, : ) );
        %lin = ( mm( 1 ) : diff( mm ) / ( np - 1 ) : mm( 2 ) )';
        lin = mm( 1 ) + diff( mm ) / ( np - 1 ) * ( 0 : ( np - 1 ) )';
        xd( t1( i ) : t2( i ) ) = x( t1( i ) : t2( i ) ) - lin;
    end
else
