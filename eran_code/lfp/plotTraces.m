...
% revisions
% 12-oct-12 calibration bar added
% 15-apr-15 oset empty case handled
% 12-aug-19 calib expanded if needed
...
x                           = x( : );
if ndims( y ) > 2
    error( '3D array plotting not supported' )
end
[ m, n ]                    = size( y );
if length( x ) ~= m
    y                       = y';
...
if nout == 0 || nout > 1
    ...
    if calib( 1 ) > 0 || calib( 2 ) > 0
        ...
        ch                  = line( x0 + [ 0 0 calib( 1 ) ], y0 + [ calib( 2 ) 0 0 ], 'color', CALIBCOLOR, 'linewidth', CALIBWIDTH );
    end
end
