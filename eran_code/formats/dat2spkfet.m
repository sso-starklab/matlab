...
% 07-mar-21 (1) modified flow control to allow spk-only or fet-only processing and updating of *.alg.# file
...
for sidx                        = 1 : nshanks
    ...
    if Overwrite == -2 && flag( 3 ) == 2
        fprintf( 1, '%s: Shank#%d spk have already been realigned and uniquified, skipping\n', mfname, ashank )
    else
        fprintf( 1, 'spk... ' )
        % extract the spk
    ...
    if Overwrite == -2 && flag( 4 ) == 2
        fprintf( 1, '%s: Shank#%d fet have already been realigned and uniquified, skipping\n', mfname, ashank )
    else
        fprintf( 1, 'fet... ' )
        % extract the fet
