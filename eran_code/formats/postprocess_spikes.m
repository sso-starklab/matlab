...
% 16-feb-21 (1) added OverwriteRealign
% 18-feb-21 (1) changed byParSST to depend on *alg* status
% 20-mar-22 (1) added call to hfoAnalysisSpiking
...
% conventions
shiftbits_DFLT          = 2^15;                 % required for conversion from int16 to uint16 (in some animals, e.g. mC41, bugs in the system can be compensated by setting this to 0)
...
% eeg2whl conversion
pix2cm_DFLT                 = 0.427;                                % room 540: 0.427; in room 549, value should be 0.366
...
[ prmOW, doCheckClu ...
    ...
    , doRealignRes, OverwriteRealign, realignResMethod ...
    , doLinearTrack ...
    , doCellType ...
    , doDigital, OverwriteDIG, shiftbits ...
    ...
    , doHFOspk, OverwriteHFOspk, OverwriteHFOphs, OverwriteHFOspkCount ...
...
if doRealignRes
    ...
    rc1                     = realignres( filebase, 'Overwrite', OverwriteRealign ...
        , 'method', realignResMethod );
    rc2                     = resunique( filebase, 'Overwrite', OverwriteRealign );
    ...
    rc3                     = dat2spkfet( filebase, 'Overwrite', OverwriteRealign ...
...
if doSST
    ...
    % first, check whether realigned and if so, nullify byParSST
    nshanks                     = par.nElecGps;
    raflag                      = zeros( nshanks, 2 );
    for ashank                  = 1 : nshanks
        algfname                = sprintf( '%s.alg.%d', filebase, ashank );
        if exist( algfname, 'file' )
           load( algfname, 'flag', '-mat' )
            raflag( ashank, : ) = flag( 3 : 4 );
        end
    end
    if all( raflag( : ) == 2 )                                              % doRealignRes has completed
        byParSST                = 0;
    end
    
    % now run_spikes_stats
...
if doHFO && doHFOspk
    ...
    % rate histograms
  stats = hfoAnalysisSpikingCount( filebase ...
        , 'Overwrite', OverwriteHFOspkCount, 'graphics', 1 ...
        ...
        for k               = 1 : length( PSTH_stimTypes )
            ...
            for i = 1 : size( PSTH_stimDurs, 1 )
                ...
                for si   = 1 : length( stimchans )
                    ...
                    [ peth, bins, trigs, tims, durs, shankclu, fig] = multipeth_make( filebase, 'ilevel', ilev, 'data', s, 'shanknums', shanknums, 'stimTypes', stimType, 'valRange', PSTH_stimVal, 'durRange', stimDur, 'channels', astimchan, 'uflag', PSTH_uflag, 'multi', PSTH_cmp );
                end
            end
        end
    end
...
if doCellType
    ...
    s = celltypeClassification( filebase,'slevel', slevel,'graphics',0, 'sourceType', {'LED','LD'}, 'Overwrite',1, 'wavRange', [ 400 500 ], 'durRange', [ 0.04 0.08 ], 'celltype',celltype, 'supFlag', supFlag, 'opsinType', 'pv::chr2', 'suffix', 's2s');
    s = celltypeClassification( filebase,'slevel', slevel,'graphics',1, 'sourceType', {'LED','LD'}, 'Overwrite',1, 'wavRange', [ 400 500 ], 'durRange', [ 0.08 0.125 ], 'celltype',celltype, 'supFlag', supFlag, 'opsinType', 'pv::chr2', 'suffix', 's2s');

    supFlag = 1; % tag by decreasing firing rate
    slevel = [0.033 0.041;0.033 0.041];
    s = celltypeClassification( filebase,'slevel', slevel,'graphics',1, 'sourceType', 'LD', 'Overwrite',1, 'wavRange', [ 550 700 ], 'durRange', [ 0.04 0.08 ], 'celltype',celltype, 'supFlag', supFlag, 'opsinType', 'pv::jaws', 'suffix', 's2s');
    s = celltypeClassification( filebase,'slevel', slevel,'graphics',1, 'sourceType', 'LD', 'Overwrite',1, 'wavRange', [ 550 700 ], 'durRange', [ 0.250 1 ], 'celltype',celltype, 'supFlag', supFlag, 'opsinType', 'pv::jaws', 'suffix', 's2s');
