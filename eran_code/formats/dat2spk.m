...
% 02-mar-21 (1) bug fix for first spike - was <0, should be <=0
%           (2) bug fix for sourcefname, was datfname in some cases
%           (3) added handling of last spike

% note: 
% 02-mar-21 does not handle properly multiple misaligned first/last spikes (yet)
...
% determine number of time samples in data
fileinfo                    = dir( sourcefname );
totsamples                  = floor( fileinfo( 1 ).bytes / nbytes / nchannels );
...
for i                       = 1 : nblocks
    ...
    if i == 1 && periods( bidx( 1 ), 1 ) <= 0 % first spike
        spk0                = readbin( sourcefname, channels, nchannels, [ 1 periods( bidx( 1 ), 2 ) ], source, target );
        spk1                = [ zeros( nchans, 1 - periods( bidx( 1 ), 1 ), target ) spk0 ];
        spk                 = readbin( sourcefname, channels, nchannels, periods( bidx( 2 : end ), : ), source, target );
        spk                 = reshape( spk, [ nchans nsamples nspikesb - 1 ] );
        spk                 = cat( 3, spk1, spk );
    elseif i == nblocks && periods( bidx( end ), 2 ) > totsamples % last spike
        spk0                = readbin( sourcefname, channels, nchannels, [ periods( bidx( end ), 1 ) totsamples ], source, target );
        spk1                = [ spk0 zeros( nchans, periods( bidx( end ), 2 ) - totsamples, target ) ];
        spk                 = readbin( sourcefname, channels, nchannels, periods( bidx( 1 : end - 1 ), : ), source, target );
        spk                 = reshape( spk, [ nchans nsamples nspikesb - 1 ] );
        spk                 = cat( 3, spk, spk1 );
