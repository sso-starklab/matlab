...
% data using e.g. 
% 
% >> figure, subplot( 2, 1, 1 ), hist( mov.pos( :, 1 ), 50 ), subplot( 2, 1, 2 ), plot( mov.pos( :, 1 ) )
% 
...
%       -runEnds        temporeal limits of LT session [mov.Fs]
...
% to do:
% (1) organize graphics for the three modes
% (2) should not translate to spkFs before end of routine
...
if graphics( 1 )
    ...
    hold on
    if ~isempty( psol1on )
        ph = plot( psol1on( :, 1 ), psol1on( :, 2 ), '.' );
        set( ph, 'color', [ 1   0 1 ] )
    end
    if ~isempty( psol1off )
        ph = plot( psol1off( :, 1 ), psol1off( :, 2 ), '.' );
        set( ph, 'color', [ 1 0.5 1 ] )
    end
    if ~isempty( psol2on )
        ph = plot( psol2on( :, 1 ), psol2on( :, 2 ), '.' );
        set( ph, 'color', [ 0   1 1 ] )
    end
    if ~isempty( psol2off )
        ph = plot( psol2off( :, 1 ), psol2off( :, 2 ), '.' );
        set( ph, 'color', [ 0.5 1 1 ] )
    end
    ...
    hold on
    if ~isempty( ppt1on )
        ph = plot( ppt1on( :, 1 ), ppt1on( :, 2 ), '.' );
        set( ph, 'color', [ 1 0 0 ] )
    end
    if ~isempty( ppt2on )
        ph = plot( ppt2on( :, 1 ), ppt2on( :, 2 ), '.' );
        set( ph, 'color', [ 0 0.7 0 ] )
    end
    if ~isempty( ppt3on )
        ph = plot( ppt3on( :, 1 ), ppt3on( :, 2 ), '.' );
        set( ph, 'color', [ 0 0 1 ] )
    end
    if ~isempty( ppt4on )
        ph = plot( ppt4on( :, 1 ), ppt4on( :, 2 ), '.' );
        set( ph, 'color', [ 1 1 0 ] )
    end
