...
%                           convType            {'median'}  argument to cch_conv
%                           hollowF             {[]}        [fraction], defaults to 1 for median, 
%                                                               0.6 for gauss (see cch_conv for more information)
...
% 01-dec-21 (1) added doDeconv flag, defaults to 1
%           (2) added hollowF, defaults to [] (see cch_conv defaults)
%           (3) act/sil by any in ROI (nBonf), not max of ROI
%           (4) hi/lo by p-values in ROI, not by global max/min
%               -for hi, also require cch > 0
%               -for lo, also require local lower > 0

% to do:
% (1) run for a few sessions, then rename this routine 'spikes2spikes.m'
% (2) apply the same algorithm (lo/hi, act/sil) to lines 278-297 of check_mono

function s2s = spikes2spikes( filebase, varargin )

% constants
deconvExpansionFactor           = 4;                                      	% expansion factor for dual deconvolution
dcMethod                        = 'fft';
...
[ isDeadTime ...
    ...
    , convType, hollowF, doDeconv, alfa ...
...
if doDeconv
    expansionFactor             = deconvExpansionFactor;
else
    expansionFactor             = 1;
end
...
if sum( nidx )
    ...
    [ pvalsUpper, pred, pvalsLower ]    = cch_conv( cchD, W, convType, hollowF ); 	% linear/non-linear filtering of the CCH
    ...
    % local lower limits, with a conservative Bonferroni correction, on the tested range:
    lcLower                  	= poissinv( alfa / nBonfD, pred( t_ROID, : ) );
    lcLowerZ                  	= poissinv( alfa / nBonfD, pred( tZ, : ) );
    
    % detect the bins with high/low local, Bonferroni-corrected counts
    hiBins                     	= false( size( cchD ) );
    loBins                     	= false( size( cchD ) );
    hiBins( t_ROID, : )        	= ( pvalsUpper( t_ROID, : ) <= alfa / nBonfD ) & ( cchD( t_ROID, : ) > 0 );
    loBins( t_ROID, : )       	= ( pvalsLower( t_ROID, : ) <= alfa / nBonfD ) & ( lcLower > 0 );
    hiBins( tZ, : )           	= ( pvalsUpper( tZ, : ) <= alfa / nBonfD ) & ( cchD( tZ, : ) > 0 );
    loBins( tZ, : )           	= ( pvalsLower( tZ, : ) <= alfa / nBonfD ) & ( lcLowerZ > 0 );
    
    % determine if any bin in t_ROI is globally sig. (binary decision)
    % act0                      = any( cchD( t_ROID, : ) >= ones( nBonfD, 1 ) * gbUpper, 1 );
    % sil0                      = any( cchD( t_ROID, : ) <= ones( nBonfD, 1 ) * gbLower, 1 );
    act0                        = any( ( pvalsUpper( t_ROID, : ) <= alfa / nBonfD ) & ( cchD( t_ROID, : ) > 0 ), 1 );
    sil0                      	= any( ( pvalsLower( t_ROID, : ) <= alfa / nBonfD ) & ( lcLower > 0 ), 1 );
...
% significance (point-wise probabilities; predictor)
[ pvalsUpper, pred, pvalsLower ] = cch_conv( cch, W, convType, hollowF ); 	% linear/non-linear filtering of the CCH
...
% local lower limits, with a conservative Bonferroni correction, on the tested range:
lcLower                         = poissinv( alfa / nBonf, pred( t_ROI, : ) );
lcLowerZ                        = poissinv( alfa / nBonf, pred( tZ, : ) );

% detect the bins with high/low local, Bonferroni-corrected counts
hiBins                          = false( size( cch ) );
loBins                          = false( size( cch ) );
hiBins( t_ROI, : )              = ( pvalsUpper( t_ROI, : ) <= alfa / nBonf ) & ( cch( t_ROI, : ) > 0 );
loBins( t_ROI, : )              = ( pvalsLower( t_ROI, : ) <= alfa / nBonf ) & ( lcLower > 0 );
hiBins( tZ, : )                 = ( pvalsUpper( tZ, : ) <= alfa / nBonf ) & ( cch( tZ, : ) > 0 );
loBins( tZ, : )                 = ( pvalsLower( tZ, : ) <= alfa / nBonf ) & ( lcLowerZ > 0 );

% determine if any bin in t_ROI is globally sig. (binary decision)
% act                             = any( cch( t_ROI, : ) >= ones( sum( t_ROI ), 1 ) * gbUpper, 1 );
% sil                             = any( cch( t_ROI, : ) <= ones( sum( t_ROI ), 1 ) * gbLower, 1 );
act                             = any( ( pvalsUpper( t_ROI, : ) <= alfa / nBonf ) & ( cch( t_ROI, : ) > 0 ), 1 );
sil                             = any( ( pvalsLower( t_ROI, : ) <= alfa / nBonf ) & ( lcLower > 0 ), 1 );

% check sum:
% [ sum( sum( hiBins( t_ROI, : ) ) > 0 ) sum( act ) ]
% [ sum( sum( loBins( t_ROI, : ) ) > 0 ) sum( sil ) ]
