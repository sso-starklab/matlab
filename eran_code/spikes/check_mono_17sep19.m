...
%                   Optional            { [ 1 1 0 ] }   pruning decisions:
...
if isa( filebase, 'char' )
    ...
    if exist( s2sfname, 'file' )
        ...
        if isequal( suffix, 's2s.nohfo' )
            s2s             = spikes2spikes( filebase, 'clustr', 'clu', 'Overwrite', -2, 'periods', -3, 'suffix', suffix );
        else
...
idx2                        = s2s.t_ROI & s2s.t == 0; % sync/de-sync (single bin)
