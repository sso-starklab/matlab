% calcPhase         compute single-frequency phase friom input
%
% call              [ phs, xf ] = calcPhase( x, fir, method, usf, graphics )
%
% gets              x             signal (vector or matrix). this can be:
...
%                   usf           {1}; upsampling factor
%                                   if integer >1, x is upsampled before phase extraction 
%                   graphics      {0}; flag
%
% returns:          phs           the phase, resolved 0-2pi. 0/2pi corresponds to peak, pi to trough
...
% calls         scale                                                           (general)
%               alines                                                          (graph)
%               fft_upsample, firfilt, local_max, makefir, my_xcorr, wavelet    (ssp) 
%
% see also      eegPhase
