...
function [ X xlta wvec xmat ] = lineRemove( X, LINE, xlta, DC, nmax, MA, w )
...
for i = 1 : length( LINE ) - 1
    ...
    nx              = t2 - t1 + 1;
    idx             = 1 : nx;
    dx              = ( xlta( nx ) - xlta( 1 ) ) / ( nx - 1 );
    slp             = cumsum( [ xlta( 1 ) ones( 1, nx - 1 ) * dx ] );
    X( t1 : t2 )    = x - xlta( idx ) + DC + slp;
    if DEBUGMODE
        clf, subplot( 4, 1, 1 ), plot( x, 'b' ), hold on, plot( X( t1 : t2 ), 'r' ), title( i ), line( xlim, [ 0 0 ], 'linestyle', '--', 'color', [ 0 0 0 ] )
        subplot( 4, 1, 2 ), plot( xlta( idx ) ), title( t1 ), line( xlim, [ 0 0 ], 'linestyle', '--', 'color', [ 0 0 0 ] )
        subplot( 4, 1, 3 ), plot( x, 'b' ), hold on, plot( x - xlta( idx ) + DC, 'r' ), title( 'no deslope' ), line( xlim, [ 0 0 ], 'linestyle', '--', 'color', [ 0 0 0 ] )
